...
write_case array_initializer_source_order.ct \
    "Contract ArrayInitializerSourceOrder:" \
    "    def main(unused: int):" \
    "        nums: int[3] = [3, 7, 1]" \
    "        acc: int = 0" \
    "        for i in Range(3):" \
...
expect_success array_initializer_source_order.ct
...
expect_failure constant_delete_identity.ct "consumed and cannot be used again"
...
# i 在循环入口已存在，Range 对 i 的赋值必须让旧常量 99 失效。
# 新 lowering 会按每轮 fixed 值裁剪条件，因此正确结果不再含运行时 IF，
# 且必须保留 i == 0 的 then 分支。
assert "OP_IF" not in tokens, tokens
assert "OP_7" in tokens, tokens
assert "OP_2" not in tokens, tokens
PY
...
# Final padding is encoded as one push.  The immutable suffix must remain
# visible after OP_RETURN and the padding in both exported representations.
tokens = lock["asm"].split()
return_pos = tokens.index("OP_RETURN")
assert tokens[:return_pos] == ["OP_1"], tokens
assert tokens[-1] == "OP_7", tokens
assert return_pos < len(tokens) - 1, tokens
assert lock["hex"].endswith("57"), lock["hex"]
assert len(lock["hex"]) > 128, lock["hex"]
PY
...
expect_ast_and_run_int array_initializer_source_order.ct 0 371
